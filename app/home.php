                            ...
                            <?php if (isset($products) && count($products)): ?> 
                                <?php foreach ($products as $product): ?>
                                    <div class="card m-1" style="width: 18rem;">
                                        <img src="<?= $product->cover ?>" class="card-img-top" alt="Producto">
                                        <div class="card-body">
                                            <h5 class="card-title"><?= htmlspecialchars($product->name) ?></h5>
                                            <p class="card-text"><?= htmlspecialchars($product->description) ?></p>
                                            
                                            <!-- Botón para ver detalles -->
                                            <a href="details.php?slug=<?= urlencode($product->slug) ?>" class="btn btn-primary">Ver detalles</a>

                                            <!-- Botón para editar producto -->
                                            <a href="edit_product.php?slug=<?= urlencode($product->slug) ?>" class="btn btn-warning">Editar</a>

                                            <!-- Botón para eliminar producto -->
                                            <form action="ProductController.php?action=delete&slug=<?= urlencode($product->slug) ?>" method="POST" class="d-inline">
